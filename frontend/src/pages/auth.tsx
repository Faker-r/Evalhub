...
export default function AuthPage() {
  ...
  const { isAuthenticated, isLoading, login, register, signInWithGoogle, authOperation } = useAuth();
  const [isLogin, setIsLogin] = useState(true);

  // Redirect if already authenticated
  useEffect(() => {
    if (!isLoading && isAuthenticated) {
      setLocation(AUTH_ROUTES.HOME);
    }
  ...
  const isEmailLoading = authOperation === 'login' || authOperation === 'register';
  const isGoogleLoading = authOperation === 'google';
  const isAnyLoading = authOperation !== 'idle';
    ...
    <div className="min-h-screen flex items-center justify-center bg-zinc-50 px-4">
      <Card className="w-full max-w-[400px] border-border shadow-lg">
        <CardHeader className="text-center pb-2">
          ...
          <CardDescription>
            {isLogin
              ? "Sign in to EvalHub to save your custom evaluations."
