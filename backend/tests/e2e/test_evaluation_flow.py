...
class TestEvaluationWorkflowE2E:
    ...
    def setup_evidence(self):
        ...
        self.evidence = {"test_run_id": datetime.now().isoformat(), "tests": []}
        yield
        # Save evidence after test
        evidence_file = (
            EVIDENCE_DIR
            / f"e2e_evidence_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
    ...
    def test_complete_benchmark_browsing_flow(self, sync_client: TestClient):
        ...
        try:
            ...
            response = sync_client.get(
                "/api/benchmarks?page=1&page_size=10&sort_by=downloads&sort_order=desc"
            )
            duration_ms = (time.time() - start) * 1000

            assert (
                response.status_code == 200
            ), f"Benchmark list failed: {response.text}"
            ...
            print(
                f"  ✓ Returned {len(benchmark_list['benchmarks'])} benchmarks on page 1"
            )
            ...
            assert (
                response.status_code == 200
            ), f"Benchmark detail failed: {response.text}"
            ...
            assert (
                response.status_code == 200
            ), f"Benchmark tasks failed: {response.text}"
    ...
    def test_authenticated_user_workflow(self, authenticated_client: TestClient):
        ...
        try:
            ...
            assert (
                response.status_code == 200
            ), f"Guidelines list failed: {response.text}"
    ...
    def test_leaderboard_data_retrieval(self, sync_client: TestClient):
        ...
        try:
            ...
            for dataset_name in test_datasets:
                ...
                start = time.time()
                response = sync_client.get(
                    f"/api/leaderboard?dataset_name={dataset_name}"
                )
                duration_ms = (time.time() - start) * 1000
                ...
                if leaderboard["entries"]:
                    ...
                    if entry.get("scores"):
                        print(
                            f"  ✓ Scores present: {len(entry['scores'])} guideline scores"
                        )
                break

            if not found_leaderboard:
                print(
                    "  ⚠ No leaderboard data found for test datasets (this is OK for test environments)"
                )
    ...
    def test_error_handling_workflow(self, sync_client: TestClient):
        ...
        try:
            ...
            if response.status_code in [401, 403]:
                ...
                print(
                    f"  ⚠ Auth middleware may be mocked (got {response.status_code}) - auth tested in integration tests"
                )
        except RuntimeError as e:
            if "different loop" in str(e):
                pytest.skip("Event loop conflict - run with live backend")
...
class TestPerformanceBaseline:
    ...
    def setup_performance_evidence(self):
        ...
        perf_file = (
            EVIDENCE_DIR
            / f"performance_evidence_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
