...
async def main() -> None:
    ...
    async with async_session() as session:
        ...
        for e in sorted(
            report.entries,
            key=lambda x: (x.model, x.provider, x.dataset_name, x.metric_name),
        ):
            print(
                f"{e.model} | {e.provider} | {e.dataset_name} | {e.metric_name} -> trace={e.trace_id} at {e.created_at} score={e.score}"
            )
