...
class CustomTaskEvaluationPipeline:
    ...
    def _accumulate_usage(self, responses: list[ModelResponse]) -> None:
        for resp in responses:
            ...
            if usage:
                self._total_token_usage["prompt_tokens"] += usage.get("prompt_tokens", 0)
                self._total_token_usage["completion_tokens"] += usage.get("completion_tokens", 0)
                self._total_token_usage["total_tokens"] += usage.get("total_tokens", 0)
